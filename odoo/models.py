...
class NewId(object):
    ...
    def __repr__(self):
        return (
            "<NewId origin=%r>" % self.origin
            if self.origin
            else "<NewId ref=%r>" % self.ref if self.ref else "<NewId 0x%x>" % id(self)
        )
...
class BaseModel(metaclass=MetaModel):
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def name_get(self):
        ...
        """
        warnings.warn(
            "Since 17.0, deprecated method, read display_name instead",
            DeprecationWarning,
            2,
        )
    ...
    def name_search(self, name="", args=None, operator="ilike", limit=100):
        ...
        if isinstance(ids, Query):
            records = self._fetch_query(
                ids, self._determine_fields_to_fetch(["display_name"])
            )
        else:
    ...
    def _name_search(self, name, domain=None, operator="ilike", limit=None, order=None):
        ...
        # optimize out the default criterion of ``like ''`` that matches everything
            ...
            domain += aggregator(
                [[(field_name, operator, name)] for field_name in search_fnames]
            )
    ...
    def _read_group(
        ...
        sql_order, sql_extra_groupby, fnames_used = self._read_group_orderby(
            order, groupby_terms, query
        )
        fnames_to_flush.update(fnames_used)
    ...
    def _read_group_select(
        ...
        if property_name and field.type != "property":
            warnings.warn(
                f"Ignore the {property_name!r} part of {aggregate_spec!r}, this notation is reserved for the Property field"
            )
    ...
    def _read_group_groupby(
        ...
        if property_name:
            if field.type != "properties":
                raise ValueError(
                    f"Property set on a non properties field: {property_name!r}"
                )
        ...
        if (
            ...
            sql_expr = SQL(
                "timezone(%s, timezone('UTC', %s))", self.env.context["tz"], sql_expr
            )
    ...
    def _read_group_orderby(
        ...
        return (
            SQL(", ").join(orderby_terms),
            SQL(", ").join(extra_groupby_terms),
            fnames_used,
        )
    ...
    def _read_group_empty_value(self, spec):
        ...
        if spec == "__count":
            return 0
        fname, __, func = parse_read_group_spec(
            spec
            ...
            for row in rows_dict:
                ...
                if not value and field.type == "many2many":
                    other_values = [
                        (
                            other_row[group][0]
                            if isinstance(other_row[group], tuple)
                            else (
                                other_row[group].id
                                if isinstance(other_row[group], BaseModel)
                                else other_row[group]
                            )
                        )
                        for other_row in rows_dict
                        if other_row[group]
                    ]
    ...
    def read_group(
        ...
        for field_spec in fields:
            ...
            if (
                ...
                and field_spec not in annoted_groupby
            ):
        ...
        if orderby:
            ...
            for order_term in orderby.split(","):
                ...
                for key_name, annoted in itertools.chain(
                    reversed(annoted_groupby.items()), annoted_aggregates.items()
                ):
        ...
        rows = self._read_group(
            domain,
            annoted_groupby.values(),
            annoted_aggregates.values(),
            offset=offset,
            limit=limit,
            order=orderby,
        )
    ...
    def _inherits_join_calc(self, alias, fname, query):
        ...
        """
        warnings.warn(
            "Deprecated method _inherits_join_calc(), _field_to_sql() instead",
            DeprecationWarning,
            2,
        )
    ...
    def _setup_base(self):
        ...
        for name, fields_ in definitions.items():
            if f"{cls._name}.{name}" in cls.pool._database_translated_fields:
                ...
                translate = next(
                    (
                        field.args["translate"]
                        for field in reversed(fields_)
                        if "translate" in field.args
                    ),
                    False,
                )
                if not translate:
                    ...
                    if anyof:
                        strs.append(
                            _(
                                ...
                                ", ".join(
                                    anyof.sorted(lambda g: g.id).mapped(
                                        lambda g: repr(g.display_name)
                                    )
                                ),
                            )
                        )
                    if noneof:
                        strs.append(
                            _(
                                ...
                                ", ".join(
                                    noneof.sorted(lambda g: g.id).mapped(
                                        lambda g: repr(g.display_name)
                                    )
                                ),
                            )
        ...
        if field.translate is True:
            ...
            for lang, translation in translations.items():
                ...
                if digest:
                    ...
                    old_terms_digested2value = {
                        digest(old_term): old_term for old_term in old_terms
                    }
                    translation = {
            ...
            if field.store:
                ...
                for dotname in self.pool.field_depends[field]:
                    ...
                    if (not dep_field.store) or (
                        ...
                        and (
                            not dep_field.groups
                            or self.user_has_groups(dep_field.groups)
                        )
                    ):
    ...
    def _fetch_query(self, query, fields):
        ...
        translated_field_names = [
            field.name for field in column_fields if field.translate
        ]
    ...
    def _check_company_domain(self, companies):
        ...
        return [
            "|",
            ("company_id", "=", False),
            ("company_id", "in", to_company_ids(companies)),
        ]
        ...
        for record in self:
            ...
            for name in regular_fields:
                ...
                if corecord:
                    ...
                    if domain and not corecord.with_context(
                        active_test=False
            ...
            if field.translate is True and val:
                ...
                assignments.append(
                    SQL(
                        ...
                        fallback=Json(
                            {}
                            if "en_US" in val.adapted
                            else {"en_US": next(iter(val.adapted.values()))}
                        ),
                        value=val,
                    )
                )
            ...
            for fname in fnames:
                ...
                if field.column_type:
                    ...
                    for stored, row in zip(stored_list, rows):
                        if fname in stored:
                            colval = field.convert_to_column(
    ...
    def _order_field_to_sql(
        ...
        if property_name and field.type != "properties":
            raise ValueError(
                f"Order a property ({property_name!r}) on a non-properties field ({field_name!r})"
            )
        ...
        if field.inherited:
            ...
            return parent_model._order_field_to_sql(
                parent_alias, full_name, direction, nulls, query
            )

        if not (field.store and field.column_type):
            _logger.warning(
                "Model %r cannot be sorted on field %r (not a column)",
                self._name,
                field_name,
            )
            return
    ...
    def _generate_order_by(self, order_spec, query):
        ...
        """
        warnings.warn(
            "Deprecated method _generate_order_by(), _order_to_sql() instead",
            DeprecationWarning,
            2,
        )
    ...
    def copy(self, default=None):
        ...
        self.with_context(from_copy_translation=True).copy_translations(
            record_copy, excluded=default or ()
        )
                            ...
                            stack.append(
                                {
                                    record.id
                                    for record in self
                                    if record.company_id.parent_ids & value_companies
                                }
                            )
                        else:
                            stack.append(
            ...
            for invf in model.pool.field_inverses[field]:
                ...
                if new_records:
                    ...
                    records |= cache_records.filtered(
                        lambda r: set(r[field.name]._ids) & set(self._ids)
                    )
...
def convert_pgerror_not_null(model, fields, info, e):
    if e.diag.table_name != model._table:
        return {
            "message": _(
                "Missing required value for the field '%s'", e.diag.column_name
            )
        }
    ...
    message = _(
        "Missing required value for the field '%s' (%s)", field["string"], field_name
    )
    return {
        "message": message,
        "field": field_name,
...
def convert_pgerror_unique(model, fields, info, e):
    ...
    if len(ufields) == 1:
        ...
        field = fields[field_name]
        message = _(
            "The value for the field '%s' already exists (this is probably '%s' in the current model).",
            field_name,
            field["string"],
        )
        return {
    ...
    message = _(
        "The values for the fields '%s' already exist (they are probably '%s' in the current model)."
